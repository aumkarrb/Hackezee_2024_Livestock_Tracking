        ...
        if (client.available()) {
            ...
            if (c == '\n') {
                ...
                if (currentLine.length() == 0) {
                    ...
                    client.println(".map { height: 200px; width: 100%; }"); // Style for map
                    client.println("</style>");
                    client.println("</head>");
                    client.println("<body>");
                    ...
                    client.println("<script async defer src='https://maps.googleapis.com/maps/api/js?key=AIzaSyDTR3JrKb_ApWGWo5AClk40xV9m3VUcHnA'></script>");
                    
                    client.println("<script async>");
                    ...
                    client.println("widget.innerHTML = `<h3>${device.ID}</h3><div class='widget-details'><p>Battery: ${device.Battery}%</p><p>Temperature: ${device.Temperature}°C</p><p>Humidity: ${device.Humidity}%</p><p>Air Quality: ${device.Air_Quality}</p><p>GPS: (${device.GPS_N.toFixed(2)}, ${device.GPS_W.toFixed(2)})</p>${warningMessage}</div><div class='map' id='map_${device.ID}'></div>`;");

                    // Initialize the map for each device
                    client.println(`initMap(${device.ID}, ${device.GPS_N}, ${device.GPS_W});`);
                    ...
                    client.println("function initMap(id, lat, lng) {");
                    client.println("  const map = new google.maps.Map(document.getElementById(`map_${id}`), {");
                    client.println("    center: { lat: lat, lng: lng },");
                    client.println("    zoom: 12");
                    client.println("  });");
                    client.println("  new google.maps.Marker({");
                    client.println("    position: { lat: lat, lng: lng },");
                    client.println("    map: map");
                    client.println("  });");
                    client.println("}");

                    // Load devices on window load
